 ...
 */
 @RunWith(classOf[JUnitRunner])
  ...
  test("max of an empty list must throw NoSuchElementException") {
    val caught = intercept[NoSuchElementException] {
    	max(List.empty)
    }
    assert(caught.getMessage().indexOf("Empty list") != -1, "'Empty list' not found in the exception message")
  }
}
