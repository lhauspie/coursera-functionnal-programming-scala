...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet {
  ...
  def filterAcc(p: Tweet => Boolean, acc: TweetSet): TweetSet = {
    val result = left.filterAcc(p, acc) union right.filterAcc(p, acc)
    if (p(elem))
      result incl elem
    else
      result
...
object GoogleVsApple {
  ...
  lazy val trending: TweetList =
    (googleTweets.filter(tweet => google.contains(tweet.text) || apple.contains(tweet.text))
      union appleTweets.filter(tweet => google.contains(tweet.text) || apple.contains(tweet.text))).descendingByRetweet
}
